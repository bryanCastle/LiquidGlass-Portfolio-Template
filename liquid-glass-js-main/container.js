class Container {
  ...
  capturePageSnapshot() {
    ...
    // Create a simplified version of the page for snapshot
    const bodyClone = document.body.cloneNode(true);
    
    // Remove all images and glass elements from the clone
    const imagesToRemove = bodyClone.querySelectorAll('img');
    imagesToRemove.forEach(img => img.remove());
    
    const glassElementsToRemove = bodyClone.querySelectorAll('.glass-container, .glass-button, .glass-button-text');
    glassElementsToRemove.forEach(el => el.remove());
